...
import { Camera, Check, X, Calendar, TrendingUp, Loader2, PenLine, Repeat, Trash2, Scale, Utensils, Pencil, LayoutDashboard } from 'lucide-react';
...
export default function App() {
  const [activeTab, setActiveTab] = useState('food'); // 'food' | 'weight' | 'calorie_trend' | 'summary'
  const [stage, setStage] = useState('idle'); // idle, comment, analyzing, confirm, error
  ...
  const tabs = [
    { key: 'food', label: '食事', icon: Utensils },
    { key: 'weight', label: '体重', icon: Scale },
    { key: 'calorie_trend', label: '推移', icon: TrendingUp },
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7F8FA', fontFamily: 'Calibri, sans-serif' }}>
      {/* タブ切り替え */}
      <div style={{ display: 'flex', background: NAVY_DARK, padding: '12px 12px 0' }}>
        {tabs.map(({ key, label, icon: Icon }) => (
          <button
            ...
            style={{
              ...
              color: activeTab === key ? '#fff' : '#8492AD',
                ...
                {entries.map((entry) => (
                  <div key={entry.id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <button onClick={() => startEditEntry(entry)} aria-label="編集" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: MUTED, display: 'flex', alignItems: 'center' }}>
                        <Pencil size={16} />
                      </button>
                      <button onClick={() => handleDelete(entry.id)} aria-label="削除" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: MUTED, display: 'flex', alignItems: 'center' }}>
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
